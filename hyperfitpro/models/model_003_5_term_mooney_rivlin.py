...
class Model003(HyperelasticModel):
    number = 3
    name = '5 term Mooney-Rivlin'
    ...
    calibration_notes = 'Higher-order polynomial; stability scan is important outside calibration range.'
    application_tags = ['higher order', 'moderate/high strain']
    material_classes = ['rubber-like elastomer']
    complexity_level = 3
    family = 'Mooney-Rivlin polynomial'
    equation_latex = 'W=C_{10}(I_1-3)+C_{01}(I_2-3)+C_{20}(I_1-3)^2+C_{11}(I_1-3)(I_2-3)'
    recommended_tests = ['uniaxial', 'biaxial', 'planar', 'simple_shear']
    notes = 'Higher-order polynomial; stability scan is important outside calibration range.'
    active = True
    parameter_specs = [
    ...
    def W(self, stretches, p):
        ...
        return p['C10']*x+p['C01']*y+p['C20']*x*x+p['C11']*x*y

MODEL_CLASS = Model003
