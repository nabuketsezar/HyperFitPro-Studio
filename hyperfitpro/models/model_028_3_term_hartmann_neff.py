...
class Model028(HyperelasticModel):
    number = 28
    name = '3 term Hartmann-Neff'
    ...
    calibration_notes = 'Requires multi-mode data to separate I1 and I2 terms.'
    application_tags = ['I1^3', 'I2^(3/2)']
    material_classes = ['rubber-like elastomer']
    complexity_level = 3
    family = 'Hartmann-Neff'
    equation_latex = 'W=\\alpha(I_1^3-3^3)+C_{10}(I_1-3)+C_{01}(I_2^{3/2}-3\\sqrt3)'
    recommended_tests = ['uniaxial', 'biaxial', 'planar']
    notes = 'Requires multi-mode data to separate I1 and I2 terms.'
    active = True
    parameter_specs = [
    ...
    def W(self, stretches, p):
        I1,I2=invariants(stretches); return p['alpha']*(I1**3-27)+p['C10']*(I1-3)+p['C01']*(safe_pow(I2,1.5)-3*np.sqrt(3))

MODEL_CLASS = Model028
